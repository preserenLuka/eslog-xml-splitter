...
export function useProcessor() {
  ...
  const addFiles = useCallback((list: File[]) => {
    ...
    // start processing
    processList(list, entries)
  }, [])

  async function processList(list: File[], entries: FileEntry[]) {
    ...
    for (let idx = 0; idx < list.length; idx++) {
      ...
      try {
        ...
        // auto-classify and collect line summaries
  ...
  const downloadAll = useCallback(async (selectedOnly: string[] | null = null) => {
    ...
    files.forEach((f) => {
      ...
      f.outputs.forEach((o) => {
        ...
        structure[`${o.type}/${name}`] = typeof o.content === 'string' ? o.content : o.content
      })
    })
