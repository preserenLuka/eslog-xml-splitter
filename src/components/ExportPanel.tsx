...
export default function ExportPanel({ files, onDownloadAll, highlightSourceId, onHoverSource, onLeave }: { files: FileEntry[], onDownloadAll: ()=>void, highlightSourceId?: string | null, onHoverSource: (id: string)=>void, onLeave: ()=>void }) {
  ...
  return (
    <div className="w-1/2 pl-4">
      ...
      <div className="mb-4">
        <button
          className="px-3 py-2 bg-green-600 text-white rounded flex items-center gap-2 hover:bg-green-700 transition-colors disabled:opacity-50"
          onClick={onDownloadAll}
          disabled={doneFiles.length===0}
        >
          <Download size={16} />
          Prenesi vse končne kot ZIP
        </button>
      </div>
              ...
              >
                ...
                <div className="mt-1">
                  <a
                    ...
                    onClick={(e)=>{
                      e.preventDefault()
                      const mime = o.type === 'report' ? 'text/plain' : 'application/xml'
                      const blob = new Blob([o.content], {type: mime})
