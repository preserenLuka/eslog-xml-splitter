...
test('realistic mixed invoice produces 166.22 EUR water and excludes 286.92 EUR waste', () => {
  ...
  expect(output.xml).toContain('<D_7008>VODARINA - N</D_7008>')
  expect(output.xml).toContain('<D_7008>OMREŽNINA VODOOSKRBA - N</D_7008>')
  expect(output.xml).toContain('<D_7008>OKOL.DAJATEV ODP.VODE, UrL 80/12</D_7008>')
  expect(output.xml).not.toContain('Osnovna tarifa')
...
test('realistic water-only invoice remains 85.06 EUR', () => {
  ...
  expect(output.xml).toContain('<D_7008>OMREŽNINA ODVAJANJE II</D_7008>')
  expect(output.xml).toContain('<D_7008>OMREŽNINA ČIŠČENJE II</D_7008>')
  expect(output.xml).toContain('<D_7008>OMREŽNINA VODOOSKRBA II</D_7008>')
  expect(output.xml).not.toContain('DN 25')
